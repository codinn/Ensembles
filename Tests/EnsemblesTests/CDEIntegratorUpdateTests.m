...
- (void)setUp
{
    ...
    // Add first event
    NSString *path = [[CDEEventStoreTestCase bundle] pathForResource:@"IntegratorUpdateTestsFixture1" ofType:@"json" inDirectory:@"Integrator Test Fixtures"];
    ...
    // Add other events
    path = [[CDEEventStoreTestCase bundle] pathForResource:@"IntegratorUpdateTestsFixture2" ofType:@"json" inDirectory:@"Integrator Test Fixtures"];
    [self addEventsFromJSONFile:path];
    [self mergeEvents];
    [self.testManagedObjectContext save:NULL];
    [self.testManagedObjectContext reset];
